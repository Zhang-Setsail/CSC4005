...
void update(float *data, float *new_data) {
    ...
    int my_begin_row_id = size * num_thread / (n_omp_threads);
    // size: 1000 rank:20 begin_id:0, 50, 100, ..., 950
    int my_end_row_id = size * (num_thread + 1) / n_omp_threads;
    if (my_begin_row_id == 0)
    {
        my_begin_row_id = 1;
    }
    if (my_end_row_id == size)
    {
        my_end_row_id = size - 1;
    }
