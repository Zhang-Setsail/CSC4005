...
typedef struct {
    ...
    int a;
    int b;
    float* data_odd_s = new float[size * size];
    float* data_even_s = new float[size * size];
    bool* fire_area_s = new bool[size * size];
    int count_m;
    //TODO END
} Args;
...
void* worker(void* args) {
    ...
    float* data_odd = my_arg->data_odd_s;
    float* data_even = my_arg->data_even_s;
    bool* fire_area = my_arg->fire_area_s;
    int count = my_arg->count_m;


    int my_begin_row_id = size * a / (b);
    // size: 1000 rank:20 begin_id:0, 50, 100, ..., 950
    int my_end_row_id = size * (a + 1) / b;
